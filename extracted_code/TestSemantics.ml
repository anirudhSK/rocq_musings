...
let print_state (ps : CrProgramState.coq_ConcreteState) =
  let header_map = ps.header_map in
  let ctrl_map = ps.ctrl_map in
  let state_map = ps.state_map in
  let header_tree = Datatypes.snd header_map in
  let ctrl_tree = Datatypes.snd ctrl_map in
  let state_tree = Datatypes.snd state_map in
  let headers = Maps.PTree.elements header_tree in
  let ctrls = Maps.PTree.elements ctrl_tree in
  let states = Maps.PTree.elements state_tree in
  let key p = Shim.coq_Z_to_int (BinNums.Zpos p) in
  let rec to_pairs = function
    | Datatypes.Coq_nil -> []
    | Datatypes.Coq_cons (Datatypes.Coq_pair (k, v), rest) ->
        (key k, Shim.crval_to_int v) :: to_pairs rest
  in
  ...
  in
  print_endline (Stdlib.String.concat "; " groups)

let run pid setup =
  let s  = setup (init_state pid) in
  Shim.run_program (get_program pid) s

let h n s = Shim.crval_to_int (Shim.get_header n s)
let v n s = Shim.crval_to_int (Shim.get_state  n s)
