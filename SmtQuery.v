...
Lemma ctrl_list_refl :
  forall c,
    ctrl_list_equal c c = true.
Proof.
  assert (forall x, ctrl_equal x x = true).
  { intros. unfold ctrl_equal. destruct x. apply Pos.eqb_refl. }
  intros. unfold ctrl_list_equal. induction c.
  - reflexivity.
  - rewrite H. simpl. assumption.
Qed.
Lemma eq_refl_lemma :
  forall [A : Type] (f : A -> A -> bool),
    ((forall x, f x x = true) -> forall y z, y = z -> f y z = true).
Proof.
...
Lemma ctrl_list_not_equal :
  forall c1 c2,
    ...
    c1 <> c2.
Proof.
  intros. intro Hcontradict.
  apply eq_refl_lemma with (f := ctrl_list_equal) in Hcontradict.
  - congruence.
  - apply ctrl_list_refl.
Qed.

Lemma hdr_list_refl :
  forall h,
    hdr_list_equal h h = true.
Proof.
  assert (forall x, header_equal x x = true).
  { intros. unfold header_equal. destruct x. apply Pos.eqb_refl. }
  intros. unfold hdr_list_equal. induction h.
  - reflexivity.
  - rewrite H. simpl. assumption.
Qed.
...
Proof.
  intros. intro Hcontradict.
  apply eq_refl_lemma with (f := hdr_list_equal) in Hcontradict.
  - congruence.
  - apply hdr_list_refl.
Qed.
...
Lemma state_list_refl :
  forall s,
    state_list_equal s s = true.
Proof.
  assert (forall x, state_equal x x = true).
  { intros. unfold state_equal. destruct x. apply Pos.eqb_refl. }
  intros. unfold state_list_equal. induction s.
  - reflexivity.
  - rewrite H. simpl. assumption.
Qed.
...
Proof.
  intros. intro Hcontradict.
  apply eq_refl_lemma with (f := state_list_equal) in Hcontradict.
  - congruence.
  - apply state_list_refl.
Qed.
...
Proof.
  intros.
  revert y' H0.
  induction x' as [|x0 xr].
  - intros.
    destruct y'.
    + reflexivity.
    + discriminate.
  - intros.
    induction y' as [|y0 yr].
    + discriminate.
    + inversion H0.
      f_equal.
      * apply H in H2. assumption.
      * apply IHxr in H3. assumption.
Qed.
...
Proof.
  intros.
  inversion H. clear H2 H3 H h1 s1 t1 h2 s2 t2.
  apply ptreelist_equal_lemma in H1.
  set (f := (fun x : Ctrl =>
    (match x with
    | CtrlCtr x_id => x_id
    end, SmtArithVar (pos_to_string match x with
    | CtrlCtr x_id => x_id
    end)))).
  apply inj_map_preserves_eq with
    (f := f)
    (x' := c1)
    (y' := c2).
  - intros.
...
Proof.
  intros.
  inversion H. clear H1 H3 H s1 c1 t1 s2 c2 t2.
  apply ptreelist_equal_lemma in H2.
  set (f := (fun x : Header =>
    (match x with
    | HeaderCtr x_id => x_id
    end, SmtArithVar (pos_to_string match x with
...
Proof.
  intros.
  inversion H. clear H1 H2 H h1 c1 t1 h2 c2 t2.
  apply ptreelist_equal_lemma in H3.
  set (f := (fun x : State =>
    (match x with
    | StateCtr x_id => x_id
    end, SmtArithVar (pos_to_string match x with
...
Definition WellFormedProgram := { p : CaracaraProgram | well_formed_program p }.

Definition pack_wfp (p : CaracaraProgram) (H : well_formed_program p) : WellFormedProgram :=
  exist _ p H.

Definition program_of_wfp (w : WellFormedProgram) : CaracaraProgram := proj1_sig w.
Definition proof_of_wfp (w : WellFormedProgram) : well_formed_program (program_of_wfp w) := proj2_sig w.
