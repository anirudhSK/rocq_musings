...
control MyIngress(inout headers hdr,
    ...
    apply {
        /* VARIANT of qos.p4: the else-if chain flattened into two independent
           ifs with explicit guards.  An `else` is the negation of the branch
           above it, so spelling that negation out has to give the same thing
           -- and the second guard is now a conjunction the lowering must carry
           into the rule rather than get from rule ORDER. */
        if (hdr.ipv4.isValid() && hdr.ipv4.protocol == IP_PROTOCOLS_UDP) {
