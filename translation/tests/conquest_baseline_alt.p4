/*
 * Princeton-Cabernet/p4-projects, ConQuest-tofino/p4src/baseline.p4, MODIFIED:
 * the ingress takes the routing byte by a shift and a mask instead of a slice.
 * Nothing else differs from conquest_baseline.p4, so the pair should be
 * equivalent; see translation/tests/README.md.
 */
// vim: syntax=P4
/*
...
control SwitchIngress(
        ...
        apply {
            bit<9> dest;
            /* VARIANT: the same top byte, as a shift and a mask rather than
               a slice.  bit<9> is wider than the byte, so no truncation is
               involved either way. */
            @in_hash{ dest= (bit<9>) ((hdr.ipv4.dst_addr >> 24) & 32w0xff); }
            route_to(dest);
        }
}
