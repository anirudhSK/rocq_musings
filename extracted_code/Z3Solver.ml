...
and z3_expr_from_coq_smt_arith_expr (expr : SmtExpr.coq_SmtArithExpr) (ctx : Z3.context) (vars : var_tracker)
  ...
  | SmtExpr.SmtArithVar name -> (
    ...
    | Some z3_var -> z3_var
    | None ->
        let z3_var = Z3.Arithmetic.Integer.mk_const ctx (Z3.Symbol.mk_string ctx name_str) in
  ...
  | SmtExpr.SmtBitAdd (e1, e2) -> Z3.Arithmetic.mk_add ctx [z3_expr_from_coq_smt_arith_expr e1 ctx vars; z3_expr_from_coq_smt_arith_expr e2 ctx vars]
  | SmtExpr.SmtBitSub (e1, e2) -> Z3.Arithmetic.mk_sub ctx [z3_expr_from_coq_smt_arith_expr e1 ctx vars; z3_expr_from_coq_smt_arith_expr e2 ctx vars]
  ...
  | SmtExpr.SmtBitMul (e1, e2) -> Z3.Arithmetic.mk_mul ctx [z3_expr_from_coq_smt_arith_expr e1 ctx vars; z3_expr_from_coq_smt_arith_expr e2 ctx vars]
  | SmtExpr.SmtBitDiv (e1, e2) -> Z3.Arithmetic.mk_div ctx (z3_expr_from_coq_smt_arith_expr e1 ctx vars) (z3_expr_from_coq_smt_arith_expr e2 ctx vars)
  | SmtExpr.SmtBitMod (e1, e2) -> Z3.Arithmetic.Integer.mk_mod ctx (z3_expr_from_coq_smt_arith_expr e1 ctx vars) (z3_expr_from_coq_smt_arith_expr e2 ctx vars)

(* Gets all variable assignments and folds into a valuation (linked list) *)
