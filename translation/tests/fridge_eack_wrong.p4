 ...
 */
/* calc_tcp_eack with the IHL scaled by two instead of four.
 *
 * Same headers, metadata, parser and deparser as fridge_eack.p4, so the two
 * allocate Headers identically and the checker compares like with like.
 *
 * (from fridge_eack.p4)
 *
...
control MyIngress(inout headers hdr, inout metadata meta, inout standard_metadata_t sm) {
    action all() {
        hdr.result.eack = hdr.tcp.seq_no
            + ((16w0 ++ hdr.ipv4.total_len)
               - (((bit<32>) (hdr.ipv4.ihl ++ 1w0))
                  + ((bit<32>) (hdr.tcp.data_offset ++ 2w0))));
    }
