...
let%expect_test "sub2_h1: h1=3 starts; after -=2" =
  let s' = run 0 (Shim.set_header_to_int 1 3) in
  Shim.print_state s';
  [%expect {| h1=1 |}]
...
let%expect_test "sub5_h1_if_h1eq0: no match, h1=3 unchanged" =
  let s' = run 1 (Shim.set_header_to_int 1 3) in
  Shim.print_state s';
  [%expect {| h1=3 |}]
...
let%expect_test "add3_h1_if_h1eq5: match fires, h1=5" =
  let s' = run 2 (Shim.set_header_to_int 1 5) in
  Shim.print_state s';
  [%expect {| h1=8 |}]
...
let%expect_test "first_match_h1eq5: only rule 1 fires" =
  let s' = run 3 (Shim.set_header_to_int 1 5) in
  Shim.print_state s';
  [%expect {| h1=6 |}]
...
let%expect_test "first_match_h1eq5: no rule matches" =
  let s' = run 3 (Shim.set_header_to_int 1 3) in
  Shim.print_state s';
  [%expect {| h1=3 |}]
...
let%expect_test "stateful_sub2: h1=10 writes s1, leaves h1" =
  let s' = run 4 (Shim.set_header_to_int 1 10) in
  Shim.print_state s';
  [%expect {|
    h1=10
    s1=8
...
let%expect_test "empty_transformer: h1=42 unchanged" =
  let s' = run 18 (Shim.set_header_to_int 1 42) in
  Shim.print_state s';
  [%expect {| h1=42 |}]
