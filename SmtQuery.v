...
Definition check_headers_and_state_vars (s1 s2 : SymbolicTransformerState)
      ...
      if varlike_list_equal h1 h2 then
        if varlike_list_equal s1 s2 then
          if varlike_list_equal c1 c2 then
            match (equivalence_checker (init_symbolic_transformer_state' p1) t1 t2 h1 s1) with
...
Lemma init_symbolic_state_nodep_t : forall h s c t1 t2,
  init_symbolic_transformer_state' (CaracaraProgramDef h s c t1) =
  init_symbolic_transformer_state' (CaracaraProgramDef h s c t2).
Proof.
  intros h s c t1 t2.
...
Class CrVarProg A := {
  ...
  lookup_var : ConcreteTransformerState -> A -> CrVal;
  ...
  equivalence_checker_cr_sound :
    ...
    let c1_i  := eval_sym_state (init_symbolic_transformer_state' p1) f in (* Get a sym state out of p1 *)
    let c2_i  := eval_sym_state (init_symbolic_transformer_state' p2) f in (* Do the same for p2 *)
...
Lemma equivalence_checker_cr_complete :
  ...
  let c1_i  := eval_sym_state (init_symbolic_transformer_state' p1) f in (* Get a sym state out of p1' headers, ctrls, and state *)
  let c2_i  := eval_sym_state (init_symbolic_transformer_state' p2) f in (* Do the same for p2 *)
  let t1 := get_transformer_from_prog p1 in
  let t2 := get_transformer_from_prog p2 in
  let c1 := eval_transformer_concrete t1 c1_i in
  let c2 := eval_transformer_concrete t2 c2_i in
  (init_symbolic_transformer_state' p1 = init_symbolic_transformer_state' p2) ->  (* both programs have the same initial symbolic state
...
Proof.
  ...
  - destruct (equivalence_checker (init_symbolic_transformer_state' (CaracaraProgramDef h1 s1 c1 (* The hard goal *)
t1)) t1 t2 h1 s1) eqn:H_eq; try (exfalso; congruence).
    -- simpl.
       intros.
       apply equivalence_checker_complete
        with (f' := f0)
             (s := init_symbolic_transformer_state' (CaracaraProgramDef h1 s1 c1 t1)) 
...
Definition build_valuation_for_cs (cs : ConcreteTransformerState) : SmtValuation :=
  fun name =>
    ...
    | Some v => v
    | None =>
      match try_match_prefix "state_" (PTree.elements (snd (t_state_map cs))) name with
      | Some v => v
      | None =>
...
Lemma valid_states_realizable :
  ...
  exists f,
  let c := eval_sym_state (init_symbolic_transformer_state' p) f in
...
Proof.
  ...
  - (* Headers *)
    ...
    + (* In p's list: cs is initialized at id, agrees with f *)
      ...
      rewrite try_match_prefix_found
        with (id := id) (w := PMap.get id (t_header_map cs)).
  ...
  - (* States *)
    ...
    + assert (Hneq : PMap.get id (t_state_map cs) <> UninitVal).
      ...
      rewrite try_match_prefix_found
        with (id := id) (w := PMap.get id (t_state_map cs)).
      * int_value_finalize Hs_valid id.
      * apply PTree.elements_keys_norepet.
      * apply PTree.elements_correct. assumption.
  ...
  - (* Ctrls *)
    ...
    + assert (Hneq : PMap.get id (t_ctrl_map cs) <> UninitVal).
      ...
      2: { intros H; inversion H. }
      rewrite try_match_prefix_found
        with (id := id) (w := PMap.get id (t_ctrl_map cs)).
      * int_value_finalize Hc_valid id.
      * apply PTree.elements_keys_norepet.
      * apply PTree.elements_correct. assumption.
...
Lemma in_program_implies_in_init_sym_header :
  forall p (v : Header),
    ...
    is_varlike_in_ps (init_symbolic_transformer_state' p) v <> None.
Proof.
  intros p v Hwf Hin.
  apply is_varlike_in_ps_lemma.
...
Proof.
  ...
  destruct (equivalence_checker (init_symbolic_transformer_state' p1) t1 t2 h1 s1) eqn:Heq; try congruence.
  clear Heq'.
  ...
  assert (Hbridge : forall (h : Header) (s : State) (c : Ctrl),
    lookup_varlike c_init h = lookup_varlike (eval_sym_state (init_symbolic_transformer_state' p1) f) h /\
    lookup_varlike c_init s = lookup_varlike (eval_sym_state (init_symbolic_transformer_state' p1) f) s /\
    lookup_varlike c_init c = lookup_varlike (eval_sym_state (init_symbolic_transformer_state' p1) f) c).
  ...
  - (* Headers case *)
    ...
    assert (Hin_ps : is_varlike_in_ps (init_symbolic_transformer_state' p1) v <> None).
    ...
    rewrite <- commute_sym_vs_conc_transformer_header_map
      with (t := t1) (f := f) (s1 := init_symbolic_transformer_state' p1) (h := v) in Hheq;
      try assumption.
    rewrite <- commute_sym_vs_conc_transformer_header_map
      with (t := t2) (f := f) (s1 := init_symbolic_transformer_state' p1) (h := v) in Hheq;
      try assumption.
  ...
  - (* States case *)
    ...
    rewrite <- commute_sym_vs_conc_transformer_state_var_map
      with (t := t1) (f := f) (s1 := init_symbolic_transformer_state' p1) (sv := v) in Hseq;
      try assumption.
    rewrite <- commute_sym_vs_conc_transformer_state_var_map
      with (t := t2) (f := f) (s1 := init_symbolic_transformer_state' p1) (sv := v) in Hseq;
      try assumption.
