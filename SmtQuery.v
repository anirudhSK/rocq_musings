...
Lemma eval_smt_bool_lemma_hdr :
  forall t1 t2 s h f,
  eval_smt_bool
(SmtBoolEq (header_map (eval_transformer_smt t1 s) h)
(header_map (eval_transformer_smt t2 s) h)) f = true ->
header_map (eval_transformer_uint8 t1 (eval_sym_state s f)) h =
header_map (eval_transformer_uint8 t2 (eval_sym_state s f)) h.
Proof.
  intros t1 t2 s h f.
  intro H.
...
Lemma eval_smt_bool_lemma_state :
  forall t1 t2 s sv f,
  eval_smt_bool
(SmtBoolEq (state_var_map (eval_transformer_smt t1 s) sv)
(state_var_map (eval_transformer_smt t2 s) sv)) f = true ->
state_var_map (eval_transformer_uint8 t1 (eval_sym_state s f)) sv =
state_var_map (eval_transformer_uint8 t2 (eval_sym_state s f)) sv.
Proof.
  intros t1 t2 s sv f.
  intro H.
  apply smt_bool_eq_true in H.
...
Lemma eval_smt_bool_lemma_hdr_false :
  ...
  eval_smt_bool
(SmtBoolEq (header_map  (eval_transformer_smt t1 s) h)
(header_map  (eval_transformer_smt t2 s) h)) f = false ->
header_map (eval_transformer_uint8 t1 (eval_sym_state s f)) h <>
header_map (eval_transformer_uint8 t2 (eval_sym_state s f)) h.
Proof.
  intros t1 t2 s h f.
...
Lemma eval_smt_bool_lemma_state_false :
  ...
  eval_smt_bool
(SmtBoolEq (state_var_map  (eval_transformer_smt t1 s) sv)
(state_var_map  (eval_transformer_smt t2 s) sv)) f = false ->
state_var_map (eval_transformer_uint8 t1 (eval_sym_state s f)) sv <>
state_var_map (eval_transformer_uint8 t2 (eval_sym_state s f)) sv.
Proof.
  intros t1 t2 s sv f.
